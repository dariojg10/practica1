...
class newForm(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        #for item in items:
            #print("Nodo: "+str(item) + " | Path Cost: " + str(item.path_cost))
        self.A.sort(key=lambda i:(i.path_cost),reverse=True)

    def pop(self):
        return self.A.pop()

class newForm2(Queue):
    """A First-In-First-Out Queue."""
    ...
    def extend(self, items):
        self.A.extend(items)
        #for item in items:
            #print("Nodo: "+str(item) + " | Path Cost: " + str(item.path_cost) + " | Heuristica: " + str(self.problem.h(item)))
        self.A.sort(key=lambda i:(i.path_cost+self.problem.h(i)),reverse=True)
